...
# and `pip install KnowMat` using an artifact store. Faster and more robust.
...
SHELL ["conda", "run", "-n", "KnowMat", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "KnowMat", "YOUR_CONSOLE_SCRIPT"]
